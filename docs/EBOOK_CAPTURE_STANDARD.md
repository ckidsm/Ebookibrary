...
> ⚠️ **크롬(chrome) 보정은 캡처 방식마다 다름 (2026-07-12 '이미지 처리 바이블' 교훈)**: `crop_page(chrome=(L,T,R,B))` 의 고정 좌우 크롭 기본값 `135`는 특정 캡처용으로, **교보 데스크탑 앱(내장 Retina) 캡처엔 과해서 본문 좌우(예: `1.1.1` 섹션번호·문단 첫 글자)를 먹었다.** 이 캡처는 창 그림자가 얇아 **`chrome=(20,150,20,155)`** 로 재크롭해 해결. **모달/화살표가 아니라 크롭이 원인이었음.** ★ 그래서 캡처 원본(`source_raws`)을 보관하면 재캡처 없이 **재크롭만으로 복구** 가능(이번에도 재캡처 0). 새 책은 첫 몇 장 크롭을 육안 확인해 크롬값을 맞춘 뒤 전량 크롭할 것.
